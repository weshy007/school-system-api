...
from school_personnel_details.permissions import IsAdminOrReadOnly
...
class StudentResultViewSet(RetrieveModelMixin, ListModelMixin, UpdateModelMixin, 
    ...
    def get_queryset(self):
        return Result.objects.select_related('unit').annotate(total=F('cat') +
            F('exam')).filter(student_id=self.kwargs['student_pk'])
